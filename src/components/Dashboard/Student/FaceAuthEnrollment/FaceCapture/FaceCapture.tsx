...
const FaceCapture = ({
  ...
  return (
    <div
      className={
        isVerification
          ? `fixed inset-0 z-50 flex items-center justify-center p-4 modal-transition`
          : `max-w-2xl mx-auto`
      }
    >
      {isVerification && (
      ...
      <div
        className={
          isVerification
            ? "relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl transform transition-all duration-300 ease-out animate-fade-in-up"
            : ""
        }
        onClick={(e) => {
          if (isVerification) e.stopPropagation();
        }}
      >
        {/* Progress bar */}
        ...
        {(currentStep === "instructions" ||
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Your face data is encrypted and stored securely. We never store
              raw images.
            </p>
          </div>
        )}
