...
import { IDeleteMaterialModalProps } from './interface';
	...
	const handleDelete = async () => {
		...
		try {
			...
			if (data.success) {
				...
				setError(
					data.message ||
						'Failed to delete material. Please check if there are any purchases associated.',
				);
			}
		} catch (err) {
				...
				<div className="p-6 space-y-4">
					...
					<div className="bg-gray-50 rounded-lg p-4 space-y-2">
						<p className="text-sm text-gray-600">
							...
							{material.course?.courseCode || 'N/A'}
						</p>
						<p className="text-sm text-gray-600">
					...
					<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
						<div className="flex items-start gap-2">
							...
							<div className="text-sm text-yellow-800">
								...
								<p>
									This material can only be deleted if no student has purchased
									it. If there are existing purchases, deletion will be blocked
									by the server.
								</p>
							</div>
						</div>
