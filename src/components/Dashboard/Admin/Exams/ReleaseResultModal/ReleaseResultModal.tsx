...
import { IAdminReleaseResultModalProps } from "./interface";
import { formatDate } from "@/src/lib/helper";
import { toast } from "react-toastify";

export default function AdminReleaseResultModal({
  isOpen,
  onClose,
  exam,
  onReleaseResult,
}: IAdminReleaseResultModalProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const releaseResults = async (examId: string) => {
    setLoading(true);
    const result = await onReleaseResult(examId);
    setLoading(false);
    if (result?.success) {
    ...
    toast.error(result?.message || "Failed to release exam results");
    setError("Failed to release exam results");
  };
    ...
    <div className="fixed inset-0 z-10 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-5 max-h-[70vh] overflow-y-auto">
          {loading ? (
            <div className="text-center py-8">Releasing Exam results...</div>
          ) : error ? (
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="font-extrabold text-gray-700 mb-2 flex items-center gap-2 animate-pulse">
                      ...
                      Performing this action would make Exam results available
                      to Students
                    </h4>
                  </div>
                </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t flex justify-evenly">
          ...
          <button
            ...
            className="px-6 py-2 bg-green-200 hover:bg-green-300 text-green-800 font-semibold rounded-xl transition"
          >
            Confirm
          </button>
        </div>
